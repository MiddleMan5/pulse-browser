    ...
    switch (action.type) {
        ...
        case types.UPDATE_TAGGROUP: {
            ...
            if (indexForEditing >= 0) {
                ...
                return [...state.slice(0, indexForEditing), modifiedEntry, ...state.slice(indexForEditing + 1)];
            }
            return state;
        }
        case types.REMOVE_TAGGROUP: {
            ...
            if (indexForRemoving >= 0) {
                return [...state.slice(0, indexForRemoving), ...state.slice(indexForRemoving + 1)];
            }
            return state;
        }
        case types.ADD_TAG: {
            ...
            if (tagIndexForUpdating >= 0) {
                ...
                return [
                    ...state.slice(0, tagGroupIndexForUpdating),
                    {
                        ...
                        children: [
                            ...state[tagGroupIndexForUpdating].children.slice(0, tagIndexForUpdating),
                            modifiedEntry,
                            ...state[tagGroupIndexForUpdating].children.slice(tagIndexForUpdating + 1),
                        ],
                    },
            ...
            if (tagIndexForRemoving >= 0) {
                return [
                    ...state.slice(0, tagGroupIndexForEditing),
                    {
                        ...
                        children: [
                            ...state[tagGroupIndexForEditing].children.slice(0, tagIndexForRemoving),
                            ...state[tagGroupIndexForEditing].children.slice(tagIndexForRemoving + 1),
                        ],
                    },
        ...
        case types.IMPORT_TAGGROUP: {
            ...
            if (action.entry[0].key) {
                ...
                action.entry.forEach((tagGroup) => {
                    ...
                    if (index > -1) {
                        tagGroup.children.forEach((tag) => {
                            const stateTag = state[index].children.find((obj) => obj.title === tag.title);
...
export const actions = {
    ...
    moveTag: (tagTitle: string, fromTagGroupUuid: Uuid, toTagGroupUuid: Uuid) => ({
        type: types.MOVE_TAG,
        tagTitle,
    ...
    exportTagGroups: (entry: Array<Object>) => (dispatch: (actions: Object) => void, getState: () => any) => {
        ...
        saveAsTextFile(blob, "tag-library [pulsebrowser " + dateTimeTag + "].json");
