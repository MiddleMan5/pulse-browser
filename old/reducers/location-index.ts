...
const Pro = undefined;
import { extractFileExtension, extractFileName, extractTagsAsObjects } from "../utils/paths";
        ...
        case types.REFLECT_RENAME_ENTRY: {
            ...
            for (let i = 0; i < GlobalSearch.index.length; i += 1) {
                if (GlobalSearch.index[i].path === action.path) {
                    ...
                    GlobalSearch.index[i].name = extractFileName(action.newPath, PlatformIO.getDirSeparator());
                    ...
                    GlobalSearch.index[i].tags = [
                        ...
                        ...extractTagsAsObjects(action.newPath, AppConfig.tagDelimiter, PlatformIO.getDirSeparator()),
                    ];
                }
            }
            return state;
...
export const actions = {
    ...
    createDirectoryIndex: (directoryPath: string, extractText: boolean, isCurrentLocation: boolean = true) => (
        ...
        createDirectoryIndex(directoryPath, extractText)
            .then((directoryIndex) => {
                ...
                if (Pro && Pro.Indexer) {
                    Pro.Indexer.persistIndex(directoryPath, directoryIndex, PlatformIO.getDirSeparator());
                }
                return true;
            })
            .catch((err) => {
            ...
            (accumulatorPromise, nextPath) =>
                accumulatorPromise.then(() =>
                    createDirectoryIndex(nextPath, extractText)
                        .then((directoryIndex) => {
                            if (Pro && Pro.Indexer) {
                                Pro.Indexer.persistIndex(nextPath, directoryIndex, PlatformIO.getDirSeparator());
                            }
                            return true;
                        })
                        .catch((err) => {
    ...
    searchLocationIndex: (searchQuery: SearchQuery) => (dispatch: (actions: Object) => void, getState: () => any) => {
        const state = getState();
        const currentLocation: Location = getLocation(state, state.app.currentLocationId);
        window.walkCanceled = false;
        if (!currentLocation) {
            dispatch(AppActions.showNotification(i18n.t("core:pleaseOpenLocation"), "warning", true));
            return;
        }
        const isCloudLocation = currentLocation.type === locationType.TYPE_CLOUD;
        dispatch(AppActions.showNotification(i18n.t("core:searching"), "default", false, "TIDSearching"));
        dispatch(actions.setSearchQuery(searchQuery));
        setTimeout(async () => {
            // Workaround used to show the start search notification
            const currentTime = new Date().getTime();
            const indexAge = GlobalSearch.indexLoadedOn ? currentTime - GlobalSearch.indexLoadedOn : 0;
            if (GlobalSearch.index.length < 1 || searchQuery.forceIndexing || indexAge > AppConfig.maxIndexAge) {
                ...
                if (currentLocation.persistIndex && Pro && Pro.Indexer.loadIndex) {
                    GlobalSearch.index = await Pro.Indexer.loadIndex(currentPath, PlatformIO.getDirSeparator());
                } else {
                    GlobalSearch.index = await createDirectoryIndex(currentPath, currentLocation.fullTextIndex);
                    if (Pro && Pro.Indexer && Pro.Indexer.persistIndex) {
                        Pro.Indexer.persistIndex(currentPath, GlobalSearch.index, PlatformIO.getDirSeparator());
                    }
                }
                ...
                .then((searchResults) => {
                    if (isCloudLocation) {
                        searchResults.forEach((entry: FileSystemEntry) => {
                            if (entry.thumbPath && entry.thumbPath.length > 1 && !entry.thumbPath.startsWith("http")) {
    ...
    searchAllLocations: (searchQuery: SearchQuery) => (dispatch: (actions: Object) => void, getState: () => any) => {
        const state = getState();
        const currentLocation: Location = getLocation(state, state.app.currentLocationId);
            ...
            (accumulatorPromise, location) =>
                accumulatorPromise.then(async () => {
                    ...
                    if (searchQuery.forceIndexing || !hasIndex) {
                        ...
                        directoryIndex = await createDirectoryIndex(nextPath, location.fullTextIndex);
                        if (Pro && Pro.Indexer && Pro.Indexer.persistIndex) {
                            Pro.Indexer.persistIndex(nextPath, directoryIndex, PlatformIO.getDirSeparator());
                        }
                    } else if (Pro && Pro.Indexer && Pro.Indexer.loadIndex) {
                        console.log("Loading index for : " + nextPath);
                        directoryIndex = await Pro.Indexer.loadIndex(nextPath, PlatformIO.getDirSeparator());
                    }
                    return Search.searchLocationIndex(directoryIndex, searchQuery)
                        .then((searchResults) => {
                            let enhancedSearchResult = searchResults;
                            if (isCloudLocation) {
                                enhancedSearchResult = searchResults.filter((entry: FileSystemEntry) => {
        ...
        result
            .then(() => {
                ...
                if (maxSearchResultReached) {
                    ...
                    dispatch(AppActions.showNotification(i18n.t("Global search completed"), "default", true));
                }
