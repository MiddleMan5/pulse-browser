...
import { loadMetaDataPromise, saveMetaDataPromise, generateFileName } from "../services/utils-io";
...
const actions = {
                ...
                processedTags.map((tag) => {
                    if (
                        taglibrary.findIndex(
                            (tagGroup) => tagGroup.children.findIndex((obj) => obj.title === tag.title) !== -1
                        ) === -1 &&
        ...
        if (!entryProperties.isFile || settings.persistTagsInSidecarFile) {
            ...
            if (fsEntryMeta) {
                const uniqueTags = getNonExistingTags(
                    tags,
                    extractTags(path, settings.tagDelimiter, PlatformIO.getDirSeparator()),
                    fsEntryMeta.tags
                );
                if (uniqueTags.length > 0) {
                ...
                saveMetaDataPromise(path, newFsEntryMeta)
                    .then(() => {
                        ...
                        const { openedFiles } = getState().app;
                        if (openedFiles.find((obj) => obj.path === path)) {
                            dispatch(AppActions.updateOpenedFile(path, { tags, changed: true }));
                        }
                        return true;
                    })
                    .catch((error) => {
            ...
            const extractedTags = extractTags(path, settings.tagDelimiter, PlatformIO.getDirSeparator());
            ...
            if (uniqueTags.length > 0) {
                ...
                const containingDirectoryPath = extractContainingDirectoryPath(path, PlatformIO.getDirSeparator());

                for (let i = 0; i < uniqueTags.length; i += 1) {
                    extractedTags.push(uniqueTags[i].title);
                }
                const newFilePath =
                    (containingDirectoryPath ? containingDirectoryPath + PlatformIO.getDirSeparator() : "") +
    ...
    editTagForEntry: (path: string, tag: Tag, newTagTitle: string) => async (
        ...
        const { settings, taglibrary } = getState();
        if (tag.functionality === "geoTagging" || tag.functionality === "dateTagging") {
        ...
        if (tag.type === "plain") {
            ...
            if (newFileName !== fileName) {
                dispatch(
                    AppActions.renameFile(path, containingDirectoryPath + PlatformIO.getDirSeparator() + newFileName)
                );
            }
        ...
        if (settings.addTagsToLibrary) {
            ...
            if (
                taglibrary.findIndex(
                    (tagGroup) => tagGroup.children.findIndex((obj) => obj.title === newTagTitle) !== -1
                ) === -1 &&
    ...
    removeTags: (paths: Array<string>, tags: Array<Tag>) => (dispatch: (actions: Object) => void) => {
        paths.map((path) => {
    ...
    removeAllTags: (paths: Array<string>) => (dispatch: (actions: Object) => void) => {
        paths.map((path) => {
            dispatch(actions.removeAllTagsFromEntry(path));
            return true;
        });
    },
    removeAllTagsFromEntry: (path: string) => (dispatch: (actions: Object) => void, getState: () => any) => {
        const { settings } = getState();
        loadMetaDataPromise(path)
            .then((fsEntryMeta) => {
                const updatedFsEntryMeta = {
                ...
                saveMetaDataPromise(path, updatedFsEntryMeta)
                    ...
                    .catch((err) => {
                        ...
                        dispatch(
                            AppActions.showNotification(i18n.t("core:removingTagsInSidecarFileFailed"), "error", true)
                        );
                        removeAllTagsFromFilename();
                    });
                return true;
    ...
    collectTagsFromLocation: (tagGroup: TagGroup) => (dispatch: (actions: Object) => void, getState: () => any) => {
        const { settings } = getState();
        ...
        const uniqueTags = Pro.Indexer.collectTagsFromIndex(GlobalSearch.index, tagGroup, settings);
        if (uniqueTags.length > 0) {
