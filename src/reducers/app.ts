...
export default (state: any = initialState, action: any) => {
    switch (action.type) {
        ...
        case types.PROGRESS: {
            const arrProgress = [{ path: action.path, progress: action.progress, abort: action.abort }];
        ...
        case types.LOAD_DIRECTORY_SUCCESS: {
            return {
                ...
                currentDirectoryColor: action.directoryMeta ? action.directoryMeta.color || "" : "",
                currentDirectoryPerspective: action.directoryMeta ? action.directoryMeta.perspective : undefined,
    ...
    return result; */
        ...
        case types.APPEND_SEARCH_RESULTS: {
            ...
            for (let i = 0; i < action.searchResults.length; i += 1) {
                const index = newDirEntries.findIndex((entry) => entry.path === action.searchResults[i].path);
                if (index === -1) {
        ...
        case types.REFLECT_CREATE_ENTRY: {
            ...
            if (
                entryIndex < 0 &&
                extractParentDirectoryPath(action.newEntry.path, PlatformIO.getDirSeparator()).replace(/[/\\]/g, "") ===
                    state.currentDirectoryPath.replace(/[/\\]/g, "")
            ) {
                return {
                    ...state,
                    currentDirectoryEntries: [...state.currentDirectoryEntries, action.newEntry],
                };
            }
            return state;
        }
            ...
            return {
                ...
                currentDirectoryEntries: state.currentDirectoryEntries.map((entry) => {
                    ...
                    return {
                        ...
                        extension: extractFileExtension(action.newPath, PlatformIO.getDirSeparator()),
                        tags: [
      ...
      return {
        ...
        case types.CLOSE_ALL_FILES: {
            ...
            window.history.pushState("", "PulseBrowser", location.pathname);
            return {
                ...state,
                openedFiles: [],
                isEntryInFullWidth: false,
...
function getNextFile(
    ...
    const currentEntries = currentDirectoryEntries ? currentDirectoryEntries.filter((entry) => entry.isFile) : [];
...
export const actions = {
    ...
    onUploadProgress: (progress, abort) => (dispatch: (actions: Object) => void) => {
        const progressPercentage = Math.round((progress.loaded / progress.total) * 100);
        console.log(progressPercentage);

        dispatch(actions.setProgress(progress.key, progressPercentage, abort));
    },
    showCreateDirectoryDialog: () => (dispatch: (actions: Object) => void, getState: () => any) => {
        const { app } = getState();
    ...
    showCreateFileDialog: () => (dispatch: (actions: Object) => void, getState: () => any) => {
        const { app } = getState();
        if (!app.currentDirectoryPath) {
            dispatch(actions.showNotification(i18n.t("core:firstOpenaFolder"), "warning", true));
        } else {
    ...
    loadParentDirectoryContent: () => (dispatch: (actions: Object) => void, getState: () => any) => {
        const state = getState();
        const { currentDirectoryPath } = state.app;
        const currentLocationPath = normalizePath(getCurrentLocationPath(state));

        if (currentDirectoryPath) {
            const parentDirectory = extractParentDirectoryPath(currentDirectoryPath, PlatformIO.getDirSeparator());
    ...
    loadDirectoryContent: (directoryPath: string) => (dispatch: (actions: Object) => void, getState: () => any) => {
        ...
        function loadDirectoryContentInt(fsEntryMeta?: FileSystemEntryMeta) {
            ...
            PlatformIO.listDirectoryPromise(directoryPath, false)
                .then((results) => {
                    prepareDirectoryContent(results, directoryPath, settings, dispatch, getState, fsEntryMeta);
                    return true;
                })
                .catch((error) => {
        ...
        loadMetaDataPromise(normalizePath(directoryPath) + PlatformIO.getDirSeparator())
            .then((fsEntryMeta) => {
    ...
    loadDirectorySuccess: (
        ...
        dispatch(actions.loadDirectorySuccessInt(directoryPath, directoryContent, false, directoryMeta));
    },
    ...
    loadDirectoryFailure: (directoryPath: string, error?: any) => (dispatch: (actions: Object) => void) => {
        console.warn("Error loading directory: " + error);
        dispatch(actions.hideNotifications());
        dispatch(actions.showNotification(i18n.t("core:errorLoadingFolder"), "warning", true));
    ...
    deleteDirectory: (directoryPath: string) => (dispatch: (actions: Object) => void, getState: () => any) => {
            ...
            .then(() => {
                if (directoryPath === currentDirectoryPath) {
                    ...
                    if (
                        ...
                        openedFiles.some(
                            (file) =>
                                extractContainingDirectoryPath(file.path, PlatformIO.getDirSeparator()) ===
                                directoryPath
                        )
                    ) {
                ...
                dispatch(
                    actions.showNotification(
                        i18n.t("deletingDirectorySuccessfull", {
                            dirPath: extractDirectoryName(directoryPath, PlatformIO.getDirSeparator()),
                        }),
                        "default",
                        true
                    )
        ...
        PlatformIO.renameDirectoryPromise(directoryPath, newDirectoryName)
            .then((newDirPath) => {
                ...
                if (currentDirectoryPath === directoryPath) {
                    ...
                    dispatch(LocationIndexActions.reflectRenameEntry(directoryPath, newDirPath));
                } else {
    ...
    createDirectory: (directoryPath: string) => (dispatch: (actions: Object) => void) => {
    ...
    createFile: () => (dispatch: (actions: Object) => void, getState: () => any) => {
        const { app } = getState();
        if (app.currentDirectoryPath) {
            const filePath =
            ...
            PlatformIO.saveFilePromise(filePath, "", true)
                .then(() => {
                    ...
                    getAllPropertiesPromise(filePath)
                        ...
                        .catch((error) =>
                            console.warn("Error getting properties for entry: " + filePath + " - " + error)
                        );
    ...
    createFileAdvanced: (targetPath: string, fileName: string, content: string, fileType: "md" | "txt" | "html") => (
        ...
        const filePath = normalizePath(targetPath) + PlatformIO.getDirSeparator() + fileNameAndExt;
        ...
        PlatformIO.saveFilePromise(filePath, fileContent, true)
            .then((fsEntry: FileSystemEntry) => {
                ...
                dispatch(actions.showNotification(`File '${fileNameAndExt}' created.`, "default", true));
                return true;
            })
            .catch((error) => {
                console.warn("Error creating file: " + error);
                dispatch(actions.showNotification(`Error creating file '${fileNameAndExt}'`, "error", true));
            });
    },
    ...
    changeLocation: (location: Location) => (dispatch: (actions: Object) => void, getState: () => any) => {
    ...
    openLocationById: (locationId: string) => (dispatch: (actions: Object) => void, getState: () => any) => {
    ...
    openLocation: (location: Location) => (dispatch: (actions: Object) => void) => {
        if (Pro && Pro.Watcher) {
    ...
    closeLocation: (locationId: string) => (dispatch: (actions: Object) => void, getState: () => any) => {
        const { locations } = getState();
        const { currentLocationId } = getState().app;
            ...
            PlatformIO.getPropertiesPromise(entryPath)
                .then((entryProps) => {
                    if (entryProps) {
                        ...
                        let entryForOpening: OpenedEntry = openedFiles.find((obj) => obj.path === entryPath);

                        if (!entryForOpening) {
                            entryForOpening = findExtensionsForEntry(supportedFileTypes, entryPath, entryProps.isFile);
                        }
    ...
    openFsEntry: (fsEntry: FileSystemEntry) => (dispatch: (actions: Object) => void, getState: () => any) => {
        ...
        entryForOpening = findExtensionsForEntry(supportedFileTypes, fsEntry.path, fsEntry.isFile);
        if (fsEntry.url) {
        ...
        // eslint-disable-next-line no-restricted-globals
        window.history.pushState("", "PulseBrowser", location.pathname + startPar);
    ...
    openNextFile: (path?: string) => (dispatch: (actions: Object) => void, getState: () => any) => {
        const nextFile = getNextFile(path, getState().app.lastSelectedEntry, getState().app.currentDirectoryEntries);
        if (nextFile !== undefined) {
    ...
    openPrevFile: (path?: string) => (dispatch: (actions: Object) => void, getState: () => any) => {
        const prevFile = getPrevFile(path, getState().app.lastSelectedEntry, getState().app.currentDirectoryEntries);
        if (prevFile !== undefined) {
    ...
    reflectDeleteEntry: (path: string) => (dispatch: (actions: Object) => void) => {
    ...
    reflectCreateEntries: (fsEntries: Array<FileSystemEntry>) => (dispatch: (actions: Object) => void) => {
        fsEntries.map((entry) => dispatch(actions.reflectCreateEntryInt(entry))); // TODO remove map and set state once
    },
    reflectCreateEntry: (path: string, isFile: boolean) => (dispatch: (actions: Object) => void) => {
        const newEntry = {
            uuid: uuidv1(),
            name: isFile
    ...
    reflectRenameEntry: (path: string, newPath: string) => (dispatch: (actions: Object) => void) => {
    ...
    reflectUpdateSidecarTags: (path: string, tags: Array<Tag>, updateIndex: boolean = true) => (
        dispatch: (actions: Object) => void,
        getState: () => any
    ) => {
        const { openedFiles } = getState().app;
    ...
    deleteFile: (filePath: string) => (dispatch: (actions: Object) => void, getState: () => any) => {
        const { settings } = getState();
        return PlatformIO.deleteFilePromise(filePath, settings.useTrashCan)
            .then(() => {
                ...
                dispatch(actions.showNotification(`Deleting file ${filePath} successful.`, "default", true));
            ...
            .catch((error) => {
                ...
                dispatch(actions.showNotification(`Error while deleting file ${filePath}`, "error", true));
            });
    },
    renameFile: (filePath: string, newFilePath: string) => (dispatch: (actions: Object) => void) =>
        PlatformIO.renameFilePromise(filePath, newFilePath)
            .then(() => {
                console.info("File renamed " + filePath + " to " + newFilePath);
                dispatch(actions.showNotification(i18n.t("core:renamingSuccessfully"), "default", true));
                dispatch(actions.reflectRenameEntry(filePath, newFilePath));
                // Update sidecar file and thumb
                renameFilesPromise([
                    [
                        getMetaFileLocationForFile(filePath, PlatformIO.getDirSeparator()),
                        getMetaFileLocationForFile(newFilePath, PlatformIO.getDirSeparator()),
                    ],
                    [
                        getThumbFileLocationForFile(filePath, PlatformIO.getDirSeparator()),
                        getThumbFileLocationForFile(newFilePath, PlatformIO.getDirSeparator()),
                    ],
                ])
                    .then(() => {
                        console.info("Renaming meta file and thumb successful from " + filePath + " to:" + newFilePath);
                        return true;
                    })
                    .catch((err) => {
                        console.warn(
            ...
            .catch((error) => {
                ...
                dispatch(actions.showNotification(`Error while renaming file ${filePath}`, "error", true));
                throw error;
            }),
...
function prepareDirectoryContent(dirEntries, directoryPath, settings, dispatch, getState, dirEntryMeta) {
    const currentLocation: Location = getLocation(getState(), getState().app.currentLocationId);
    const isCloudLocation = currentLocation.type === locationType.TYPE_CLOUD;

    const { directoryContent, tmbGenerationPromises, tmbGenerationList } = enhanceDirectoryContent(
        dirEntries,
        isCloudLocation,
    ...
    if (tmbGenerationPromises.length > 0) {
        dispatch(actions.setGeneratingThumbnails(true));
        Promise.all(tmbGenerationPromises).then(handleTmbGenerationResults).catch(handleTmbGenerationFailed);
    }

    console.log("Dir " + directoryPath + " contains " + directoryContent.length);
    dispatch(actions.loadDirectorySuccess(directoryPath, directoryContent, dirEntryMeta));
}
...
function findExtensionsForEntry(supportedFileTypes: Array<any>, entryPath: string, isFile = true): OpenedEntry {
    const fileExtension = extractFileExtension(entryPath, PlatformIO.getDirSeparator()).toLowerCase();
    const viewingExtensionPath = isFile ? findExtensionPathForId("@pulsebrowser/text-viewer") : "about:blank";
    ...
    supportedFileTypes.map((fileType) => {
        if (fileType.viewer && fileType.type.toLowerCase() === fileExtension) {
            ...
            fileForOpening.viewingExtensionPath = findExtensionPathForId(fileType.viewer);
            if (fileType.editor && fileType.editor.length > 0) {
                fileForOpening.editingExtensionId = fileType.editor;
                fileForOpening.editingExtensionPath = findExtensionPathForId(fileType.editor);
            }
        }
        return true;
    });
...
export function findAvailableExtensions() {
    const extensionsFound = [
        {
            extensionId: "@pulsebrowser/archive-viewer",
            extensionName: "Archive Viewer",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/document-viewer",
            extensionName: "Documents Viewer",
            extensionType: "viewer",
        },
        // { extensionId: '@pulsebrowser/ebook-viewer', extensionName: 'EPUB Viewer', extensionType: 'viewer' },
        {
            extensionId: "@pulsebrowser/html-editor",
            extensionName: "HTML Editor",
            extensionType: "editor",
        },
        {
            extensionId: "@pulsebrowser/html-viewer",
            extensionName: "HTML Viewer",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/image-viewer",
            extensionName: "Image Viewer",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/json-editor",
            extensionName: "JSON Viewer",
            extensionType: "editor",
        },
        {
            extensionId: "@pulsebrowser/md-viewer",
            extensionName: "MarkDown Viewer",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/media-player",
            extensionName: "Media Player",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/mhtml-viewer",
            extensionName: "MHTML Viewer",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/pdf-viewer",
            extensionName: "PDF Viewer",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/plain-viewer",
            extensionName: "Experimental Viewer - insecure!!!",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/rtf-viewer",
            extensionName: "RTF Viewer",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/text-editor",
            extensionName: "Text Editor",
            extensionType: "editor",
        },
        {
            extensionId: "@pulsebrowser/text-viewer",
            extensionName: "Text Viewer",
            extensionType: "viewer",
        },
        {
            extensionId: "@pulsebrowser/url-viewer",
...
// Selectors
export const getDirectoryContent = (state: any) => state.app.currentDirectoryEntries;
export const getCurrentDirectoryColor = (state: any) => state.app.currentDirectoryColor;
export const getCurrentDirectoryPerspective = (state: any) => state.app.currentDirectoryPerspective;
...
export const isOnboardingDialogOpened = (state: any) => state.app.onboardingDialogOpened;
export const isEditTagDialogOpened = (state: any) => state.app.editTagDialogOpened;
...
export const isLicenseDialogOpened = (state: any) => state.app.licenseDialogOpened;
export const isThirdPartyLibsDialogOpened = (state: any) => state.app.thirdPartyLibsDialogOpened;
export const isSettingsDialogOpened = (state: any) => state.app.settingsDialogOpened;
export const isCreateDirectoryOpened = (state: any) => state.app.createDirectoryDialogOpened;
export const isCreateFileDialogOpened = (state: any) => state.app.createFileDialogOpened;
export const isSelectDirectoryDialogOpened = (state: any) => state.app.selectDirectoryDialogOpened;
export const isUploadDialogOpened = (state: any) => state.app.uploadDialogOpened;
...
export const getNotificationStatus = (state: any) => state.app.notificationStatus;
export const getSearchResultCount = (state: any) => state.app.currentDirectoryEntries.length;
...
export const isLocationManagerPanelOpened = (state: any) => state.app.locationManagerPanelOpened;
export const isTagLibraryPanelOpened = (state: any) => state.app.tagLibraryPanelOpened;
export const isSearchPanelOpened = (state: any) => state.app.searchPanelOpened;
export const isPerspectivesPanelOpened = (state: any) => state.app.perspectivesPanelOpened;
export const isHelpFeedbackPanelOpened = (state: any) => state.app.helpFeedbackPanelOpened;
