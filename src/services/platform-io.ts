...
const Pro = undefined;
import NativePlatformIO from "./electron-io";
...
export default class PlatformIO {
        ...
        new Promise((resolve, reject) => {
            if (Pro) {
                if (
                    ...
                    objectStoreAPI.config.secretAccessKey === objectStoreConfig.secretAccessKey &&
    ...
    static getDirSeparator = (): string => (PlatformIO.haveObjectStoreSupport() ? "/" : AppConfig.dirSeparator); // TODO rethink usage for S3 on Win
    ...
    static watchDirectory = (dirPath: string, listener): void => nativeAPI.watchDirectory(dirPath, listener);
    ...
    static createDirectoryTree = (directoryPath: string): Object => nativeAPI.createDirectoryTree(directoryPath);
    ...
    static createThumbnailsInWorker = (tmbGenerationList: Array<string>): Promise<any> =>
        nativeAPI.createThumbnailsInWorker(tmbGenerationList);
    ...
    static copyFilePromise = (sourceFilePath: string, targetFilePath: string): Promise<any> => {
        if (objectStoreAPI) {
    ...
    static renameFilePromise = (filePath: string, newFilePath: string): Promise<any> => {
        if (objectStoreAPI) {
    ...
    static renameDirectoryPromise = (dirPath: string, newDirName: string): Promise<any> => {
        if (objectStoreAPI) {
    ...
    static loadTextFilePromise = (filePath: string, isPreview?: boolean): Promise<any> => {
        if (objectStoreAPI) {
    ...
    static getFileContentPromise = (filePath: string, type?: string): Promise<Object> => {
        if (objectStoreAPI) {
    ...
    static saveFilePromise = (filePath: string, content: any, overwrite: boolean): Promise<any> => {
        if (objectStoreAPI) {
    ...
    static saveTextFilePromise = (filePath: string, content: string, overwrite: boolean): Promise<any> => {
        if (objectStoreAPI) {
    ...
    static saveBinaryFilePromise = (
        ...
        if (objectStoreAPI) {
            return objectStoreAPI.saveBinaryFilePromise(filePath, content, overwrite, onUploadProgress);
        }
        return nativeAPI.saveBinaryFilePromise(filePath, content, overwrite).then((succeeded) => {
    ...
    static deleteDirectoryPromise = (path: string, useTrash?: boolean): Promise<any> => {
        if (objectStoreAPI) {
    ...
    static openDirectory = (dirPath: string): void => nativeAPI.openDirectory(dirPath);

    static showInFileManager = (dirPath: string): void => nativeAPI.showInFileManager(dirPath);
    ...
    static selectDirectoryDialog = (): Promise<any> => nativeAPI.selectDirectoryDialog();
