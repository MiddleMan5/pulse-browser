/**
 * PulseBrowser - universal file and folder organizer
 * Copyright (C) 2017-present PulseBrowser UG (haftungsbeschraenkt)
 *
...
export default class ElectronIO {
    ...
    initTrayMenu = (menuConfig: Array<Object>) => {
        ...
        this.tsTray.setToolTip("PulseBrowser");
    ...
    watchDirectory = (dirPath: string, listener: Object): void => {
        ...
        this.fsWatcher = this.fs.watch(dirPath, { persistent: true, recursive: false }, listener);
    };
    ...
    getAppDataPath = (): string => this.ipcRenderer.sendSync("app-data-path-request", "notNeededArgument");
    ...
    getAppPath = (): string => this.ipcRenderer.sendSync("app-dir-path-request", "notNeededArgument");
        ...
        const generateDirectoryTree = (dirPath: string) => {
            try {
                ...
                for (const dir of dirList) {
                    const path = dirPath + AppConfig.dirSeparator + dir;
    ...
    listDirectoryPromise = (path: string, lite = true, extractTextContent = false): Promise<Array<Object>> =>
        new Promise((resolve) => {
                ...
                if (entries) {
                    ...
                    if (!lite && containsMetaFolder) {
                        ...
                        this.fs.readdir(metaFolderPath, (err, metaEntries) => {
                            if (err) {
                                console.log("Error listing meta directory " + metaFolderPath + " - " + err);
                                resolve(enhancedEntries); // returning results even if any promise fails
                                return;
                            }
    ...
    getPropertiesPromise = (path: string): Promise<any> =>
        new Promise((resolve) => {
            ...
            this.fs.lstat(path, (err, stats) => {
                ...
                if (stats) {
                    resolve({
                        name: path.substring(path.lastIndexOf(AppConfig.dirSeparator) + 1, path.length),
                        isFile: stats.isFile(),
                        size: stats.size,
    ...
    copyFilePromise = (sourceFilePath: string, targetFilePath: string): Promise<any> => {
        console.log("Copying file: " + sourceFilePath + " to " + targetFilePath);
        return new Promise((resolve, reject) => {
            if (sourceFilePath === targetFilePath) {
                reject('Trying to copy over the same file. Copying "' + sourceFilePath + '" failed');
    ...
    renameFilePromise = (filePath: string, newFilePath: string): Promise<any> => {
        ...
        return new Promise((resolve, reject) => {
            if (filePath === newFilePath) {
                reject('Source and target file paths are the same. Renaming of "' + filePath + '" failed');
                return;
            }
            if (this.fs.lstatSync(filePath).isDirectory()) {
                reject('Trying to rename a directory. Renaming of "' + filePath + '" failed');
                return;
            }
            if (!this.fs.existsSync(filePath)) {
                reject('Source file does not exist. Renaming of "' + filePath + '" failed');
                return;
            }
            if (this.fs.existsSync(newFilePath)) {
                reject('Target filename "' + newFilePath + '" exists. Renaming of "' + filePath + '" failed');
                return;
            }
    ...
    renameDirectoryPromise = (dirPath: string, newDirName: string): Promise<any> => {
        const newDirPath =
            extractParentDirectoryPath(dirPath, AppConfig.dirSeparator) + AppConfig.dirSeparator + newDirName;
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.fs.existsSync(newDirPath)) {
                reject('Directory "' + newDirPath + '" exists. Renaming of "' + dirPath + '" failed');
                return;
            }
    ...
    loadTextFilePromise = (filePath: string, isPreview: boolean = false): Promise<string> =>
        new Promise((resolve, reject) => {
            if (isPreview) {
    ...
    saveFilePromise = (filePath: string, content: any, overwrite: boolean = true): Promise<FileSystemEntry> =>
        new Promise((resolve, reject) => {
            ...
            this.getPropertiesPromise(filePath)
                ...
                .catch((error) => {
                    ...
                    console.log("Getting properties for " + filePath + " failed with: " + error);
                    const fsEntry = {
    ...
    saveTextFilePromise = (filePath: string, content: string, overwrite: boolean): Promise<FileSystemEntry> => {
    ...
    saveBinaryFilePromise = (filePath: string, content: any, overwrite: boolean): Promise<FileSystemEntry> => {
        console.log("Saving binary file: " + filePath);
        const buff = arrayBufferToBuffer(content);
        return this.saveFilePromise(filePath, buff, overwrite);
    };

    deleteFilePromise = (path: string, useTrash: boolean = true): Promise<any> => {
        if (useTrash) {
            return new Promise((resolve, reject) => {
    ...
    deleteDirectoryPromise = (path: string, useTrash: boolean = true): Promise<any> => {
        if (useTrash) {
            return new Promise((resolve, reject) => {
                this.moveToTrash([path])
                    .then(() => resolve(path))
    ...
    openDirectory = (dirPath: string): void => {
        if (AppConfig.isWin) {
            ...
            this.electron.shell.showItemInFolder(dirPath + AppConfig.dirSeparator + ".");
        }
    };
