...
const versionMeta = { version, name };
...
export function enhanceEntry(entry: any): FileSystemEntry {
    let fileNameTags = [];
    if (entry.isFile) {
        fileNameTags = extractTagsAsObjects(entry.name, AppConfig.tagDelimiter, PlatformIO.getDirSeparator());
    }
    ...
    const enhancedEntry: FileSystemEntry = {
        ...
        name: entry.name,
        isFile: entry.isFile,
        extension: entry.isFile ? extractFileExtension(entry.name, PlatformIO.getDirSeparator()) : "",
...
export function enhanceOpenedEntry(entry: OpenedEntry, tagDelimiter): OpenedEntry {
    const fineNameTags = extractTagsAsObjects(entry.path, tagDelimiter, PlatformIO.getDirSeparator());
        ...
        setTimeout(() => {
            ...
            walkDirectory(
                ...
                .then(() => {
                    ...
                    console.log("Directory index created " + dirPath + " containing " + SearchIndex.length);
...
export function walkDirectory(path: string, options: Object = {}, fileCallback: any, dirCallback: any) {
    const mergedOptions = {
        recursive: false,
        skipMetaFolder: true,
        ...
        .then((entries) => {
            ...
            return Promise.all(
                entries.map((entry) => {
                    ...
                    if (entry.isFile) {
                        if (fileCallback && (!mergedOptions.skipDotHiddenFiles || !entry.name.startsWith("."))) {
                            fileCallback(entry);
                        }
                        return entry;
                    }
...
export function generateFileName(fileName: string, tags: Array<string>, tagDelimiter: string) {
    ...
    if (beginTagContainer < 0 || endTagContainer < 0 || beginTagContainer >= endTagContainer) {
        ...
        if (lastDotPosition < 0) {
            ...
            newFileName =
                fileName.substring(0, lastDotPosition).trim() + prefixTagContainer + tagsString + "." + fileExt;
        }
    } else {
        // File does not have an extension
        newFileName =
...
export async function loadMetaDataPromise(path: string): Promise<FileSystemEntryMeta> {
    ...
    if (entryProperties.isFile) {
        const metaFilePath = getMetaFileLocationForFile(path, PlatformIO.getDirSeparator());
        let metaData;
        try {
        ...
        const metaFilePath = getMetaFileLocationForDir(path, PlatformIO.getDirSeparator());
        let metaData;
        try {
            metaData = await loadJSONFile(metaFilePath);
        } catch (e) {
...
export async function saveMetaDataPromise(path: string, metaData: any): Promise<any> {
    const entryProperties = await PlatformIO.getPropertiesPromise(path);
    if (entryProperties) {
        let metaFilePath;
        let newFsEntryMeta;
        if (entryProperties.isFile) {
            metaFilePath = getMetaFileLocationForFile(path, PlatformIO.getDirSeparator());
            ...
            const metaDirectoryPath = getMetaDirectoryPath(path, PlatformIO.getDirSeparator());
            ...
            metaFilePath = getMetaFileLocationForDir(path, PlatformIO.getDirSeparator());
            newFsEntryMeta = {
                ...metaData,
                id: dirId,
                appName: versionMeta.name,
    ...
    return new Promise((resolve, reject) => reject(new Error("file not found" + path)));
}

export function findColorForFileEntry(fileExtension: string, isFile: boolean, supportedFileTypes: Array<any>): string {
    if (!isFile) {
...
export function loadFileContentPromise(fullPath: string, type: string): Promise<any> {
    return new Promise((resolve, reject) => {
