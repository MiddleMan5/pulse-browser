...
function getThumbFileLocation(filePath: string) {
    const containingFolder = extractContainingDirectoryPath(filePath, PlatformIO.getDirSeparator());
    const metaFolder = getMetaDirectoryPath(containingFolder, PlatformIO.getDirSeparator());
    return (
        metaFolder +
...
export function getThumbnailURLPromise(filePath: string): Promise<{ filePath: string; tmbPath?: string }> {
...
export function createThumbnailPromise(filePath: string, fileSize: number, thumbFilePath: string): Promise<any> {
    return new Promise(async (resolve) => {
        const metaDirectory = extractContainingDirectoryPath(thumbFilePath, PlatformIO.getDirSeparator());
        const fileDirectory = extractContainingDirectoryPath(filePath, PlatformIO.getDirSeparator());
...
export function generateThumbnailPromise(fileURL: string, fileSize: number) {
    const ext = extractFileExtension(fileURL, PlatformIO.getDirSeparator()).toLowerCase();
