...
export default class ObjectStoreIO {
    ...
    configure = (objectStoreConfig: Object): Promise<any> =>
        new Promise((resolve, reject) => {
            ...
            import(/* webpackChunkName: "AWS" */ "aws-sdk")
                .then(({ default: AWS }) => {
                    const advancedMode = this.config.endpointURL && this.config.endpointURL.length > 7;
                    if (advancedMode) {
    ...
    listDirectoryPromise = (path: string, lite: boolean = true): Promise<Array<Object>> =>
        new Promise(async (resolve) => {
            ...
            const params = {
                Delimiter: "/", // '/',
                Prefix: path.length > 0 && path !== "/" ? normalizePath(this.normalizeRootPath(path)) + "/" : "",
       ...
       */
                ...
                data.Contents.forEach((file) => {
                    ...
                    const thumbAvailable = metaContent.find((obj: any) => obj.path === thumbPath);
                    if (thumbAvailable) {
                    ...
                    if (file.Key !== params.Prefix) {
                        ...
                        const metaFileAvailable = metaContent.find((obj: any) => obj.path === metaFilePath);
        ...
        const promise = new Promise(async (resolve) => {
            if (eentry.isFile) {
                ...
                if (
                    ...
                    const folderTmbPath = eentry.path + AppConfig.metaFolder + "/" + AppConfig.folderThumbFile;
                    const folderThumbProps = await this.getPropertiesPromise(folderTmbPath);
                    ...
                    if (folderProps.isFile) {
                        const metaFileContent = await this.loadTextFilePromise(folderMetaPath);
                        eentry.meta = JSON.parse(metaFileContent.trim());
    ...
    getPropertiesPromise = (path: string): Promise<any> =>
                    ...
                    if (err) {
                        ...
                        this.objectStore.listObjectsV2(listParams, (listError, listData) => {
                            ...
                            const folderExists =
                                ...
                                (listData && listData.CommonPrefixes && listData.CommonPrefixes.length > 0); // needed for DO
                            if (folderExists) {
                                resolve({
        ...
        */
                    ...
                    resolve({
                        name: isFile ? extractFileName(normalizedPath, "/") : extractDirectoryName(normalizedPath, "/"),
                        isFile,
    ...
    loadTextFilePromise = (filePath: string, isPreview?: boolean): Promise<string> =>
        this.getFileContentPromise(filePath, "text", isPreview);
     ...
     * @returns {Promise<any>}
     */
    getFileContentPromise = async (filePath: string, type: string, isPreview?: boolean): Promise<any> => {
     ...
     */
    saveFilePromise = (filePath: string, content: string, overWrite: boolean, mode: string): Promise<Object> =>
        new Promise((resolve, reject) => {
            let isNewFile = false;
    ...
    normalizeRootPath(filePath: string) {
        filePath = filePath.replace(new RegExp(AppConfig.dirSeparator + AppConfig.dirSeparator + "+", "g"), "/");
    ...
    saveBinaryFilePromise(
        ...
        content: string,
        overWrite: boolean,
        onUploadProgress?: (progress: ManagedUpload.Progress, request: () => any) => void,
        onAbort?: () => void
            ...
            this.getPropertiesPromise(filePath)
                .then((result) => {
                    ...
                    if (isNewFile || overWrite === true) {
                        ...
                        if (onUploadProgress) {
                            request.on("httpUploadProgress", (progress: ManagedUpload.Progress) => {
                                if (onUploadProgress) {
    ...
    renameDirectoryPromise = (dirPath: string, newDirectoryPath: string): Promise<Object> => {
        const newDirPath = extractParentDirectoryPath(dirPath, "/") + "/" + newDirectoryPath;
    ...
    openDirectory = (dirPath: string) => {
        console.log("Opening directory " + dirPath + " not possible for the s3 version.");
    };
