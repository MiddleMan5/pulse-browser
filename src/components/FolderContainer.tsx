...
import { getPerspectives, getMaxSearchResults, getDesktopMode } from "../reducers/settings";
...
import { enhanceOpenedEntry, FileSystemEntry, FileSystemEntryMeta } from "../services/utils-io";
import AppConfig from "../config";

const GridPerspective = React.lazy(
    () => import(/* webpackChunkName: "GridPerspective" */ "../perspectives/grid-perspective/")
);
...
const GalleryPerspective = React.Fragment;
...
const MapiquePerspective = React.Fragment;
...
const KanBanPerspective = React.Fragment;
...
const FolderContainer = (props: Props) => {
    ...
    boolean
  >(false); */
    const [directoryContextMenuAnchorEl, setDirectoryContextMenuAnchorEl] = useState<null | HTMLElement>(null);
    ...
    useEffect(() => {
        if (props.openedFiles.length > 0) {
            ...
            if (openedFile.path === props.currentDirectoryPath) {
                ...
                const currentEntry = enhanceOpenedEntry(openedFile, props.settings.tagDelimiter);
    ...
    if (props.currentDirectoryPath) {
        ...
        let normalizedCurrentPath = addSlash + normalizePath(props.currentDirectoryPath.split("\\").join("/"));

        let normalizedCurrentLocationPath = "";
        if (props.currentLocationPath) {
            normalizedCurrentLocationPath = addSlash + normalizePath(props.currentLocationPath.split("\\").join("/"));
        }
    ...
    const renderPerspective = () => {
        if (!props.currentDirectoryPath && props.directoryContent.length < 1) {
            return AppConfig.showWelcomePanel ? <WelcomePanelAsync /> : <React.Fragment />;
        }
        if (Pro && props.currentDirectoryPerspective === Pro.Perspectives.AvailablePerspectives.GALLERY) {
            return (
      ...
      return (
        ...
        if (Pro && props.currentDirectoryPerspective === Pro.Perspectives.AvailablePerspectives.MAPIQUE) {
            return (
        ...
        if (Pro && props.currentDirectoryPerspective === Pro.Perspectives.AvailablePerspectives.KANBAN) {
            return (
    ...
    if (searchResultCount >= maxSearchResults) {
        searchResultCounterText =
            "Max. search count reached, showing only the first " + searchResultCount + " entries.";
    }
    return (
        <div data-tid="folderContainerTID">
            <div className={classes.mainPanel}>
                        ...
                        <React.Fragment>
                            ...
                            {props.currentDirectoryPath && (
                                <React.Fragment>
                                    ...
                                    >
                                        {extractShortDirectoryName(normalizePath(normalizedCurrentDirPath), "/")}
                                        <MoreVertIcon />
                                    </Button>
                                    <DirectoryMenu
                ...
                <div className={classes.centerPanel} style={{ height: props.windowHeight }}>
                    {renderPerspective()}
                </div>
            </div>
        </div>
...
export default connect(mapStateToProps, mapActionCreatorsToProps)(withStyles(styles)(withTheme(FolderContainer)));
