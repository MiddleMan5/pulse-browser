...
function getItemStyles(initialOffset: XYCoord | null, currentOffset: XYCoord | null, clientOffset: XYCoord | null) {
...
export const CustomDragLayer: React.FC<CustomDragLayerProps> = (props) => {
    ...
    function renderItem() {
        switch (itemType) {
            case DragItemTypes.FILE:
                return <FilesDragPreview entries={item.selectedEntries} path={item.path} />;
    ...
    return (
        <div style={layerStyles}>
            <div style={getItemStyles(initialOffset, currentOffset, clientOffset)}>{renderItem()}</div>
        </div>
    );
};
