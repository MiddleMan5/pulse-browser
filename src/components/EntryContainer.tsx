...
import { OpenedEntry, NotificationTypes, isReadOnlyMode, actions as AppActions } from "../reducers/app";
...
const EntryContainer = (props: Props) => {
    ...
    const [isPropertiesPanelVisible, setPropertiesPanelVisible] = useState<boolean>(false);
    const [isFullscreen, setFullscreen] = useState<boolean>(false);
    // const [editingSupported, setEditingSupported] = useState<boolean>(true);
    const [isSaveBeforeCloseConfirmDialogOpened, setSaveBeforeCloseConfirmDialogOpened] = useState<boolean>(false);
    const [isSaveBeforeReloadConfirmDialogOpened, setSaveBeforeReloadConfirmDialogOpened] = useState<boolean>(false);
    const [isEditTagsModalOpened, setEditTagsModalOpened] = useState<boolean>(false);
    const [isDeleteEntryModalOpened, setDeleteEntryModalOpened] = useState<boolean>(false);
    ...
    useEventListener("message", (e) => {
        if (typeof e.data === "string") {
            ...
            try {
                ...
                console.debug("useEventListener message:" + e.data + " parse error:", ex);
            }
        }
    });
    ...
    useEffect(() => {
        ...
        setPropertiesPanelVisible(true);
            ...
            if (openedFile.editMode && openedFile.changed && openedFile.shouldReload === false) {
     ...
     *  always open for dirs
     */
    const isPropPanelVisible = openedFile.isFile ? isPropertiesPanelVisible : true;
    ...
    const handleMessage = (data: any) => {
        ...
        switch (data.command) {
            ...
            case "loadDefaultTextContent":
                ...
                // }
                PlatformIO.loadTextFilePromise(textFilePath, data.preview ? data.preview : false)
                    .then((content) => {
                        let fileDirectory = extractContainingDirectoryPath(textFilePath, PlatformIO.getDirSeparator());
                        if (AppConfig.isWeb) {
                            fileDirectory =
                                extractContainingDirectoryPath(
                        ...
                        if (
                            ...
                            fileViewer.current.contentWindow.setContent(content, fileDirectory, !openedFile.editMode);
                        }
                        return true;
                    })
                    .catch((err) => {
            ...
            default:
                console.log("Not recognized messaging command: " + JSON.stringify(data));
                break;
        }
    };
    ...
    const saveFile = (textContent: string) => {
        PlatformIO.saveTextFilePromise(openedFile.path, textContent, true)
            .then((result) => {
                ...
                props.showNotification(i18n.t("core:fileSavedSuccessfully"), NotificationTypes.default);
                return result;
            })
            .catch((error) => {
                props.showNotification(i18n.t("core:errorSavingFile"), NotificationTypes.error);
    ...
    const toggleFullScreen = () => {
      ...
      return;
        ...
        if (!isFullscreen && fileViewerContainer) {
            if (fileViewerContainer && fileViewerContainer.current.webkitRequestFullscreen) {
                fileViewerContainer.current.webkitRequestFullscreen();
                setFullscreen(true);
            } else if (fileViewerContainer && fileViewerContainer.current.requestFullscreen) {
                fileViewerContainer.current.requestFullscreen();
                setFullscreen(true);
            } else if (fileViewerContainer && fileViewerContainer.current.mozRequestFullScreen) {
        ...
        <div className={classes.toolbar2}>
            <div className={classes.flexLeft}>
                ...
                <IconButton
                    ...
                    onClick={() => {
                        const entryName = `${baseName(openedFile.path, PlatformIO.getDirSeparator())}`;
                        ...
                        if (AppConfig.isCordova) {
                            if (openedFile.url) {
                                ...
                                props.showNotification(i18n.t("core:cantDownloadLocalFile"), NotificationTypes.default);
                            }
                        } else {
                            ...
                            if (downloadLink) {
                                if (AppConfig.isWeb) {
                                    ...
                                    downloadLink.setAttribute("href", `file:///${openedFile.path}`);
                                }
    ...
    if (openedFile.path) {
        fileTitle = extractTitle(openedFile.path, !openedFile.isFile, PlatformIO.getDirSeparator());
    ...
    function getSplitPanelSize() {
        if (isPropPanelVisible) {
            return openedFile.isFile ? props.settings.entryPropertiesSplitSize : "100%";
        }
        ...
        >
            ...
            {isDeleteEntryModalOpened && (
                <ConfirmDialog
                    ...
                    title={openedFile.isFile ? i18n.t("core:deleteConfirmationTitle") : i18n.t("core:deleteDirectory")}
                    content={
                        openedFile.isFile
                            ? i18n.t("core:doYouWantToDeleteFile")
                            : i18n.t("core:deleteDirectoryContentConfirm", {
                                  dirPath: openedFile.path
                                      ? extractDirectoryName(openedFile.path, PlatformIO.getDirSeparator())
                                      : "",
                              })
                    }
                    confirmCallback={(result) => {
            ...
            <SplitPane
                ...
                defaultSize={openedFile.isFile ? props.settings.entryPropertiesSplitSize : "100%"}
                onChange={(size) => {
