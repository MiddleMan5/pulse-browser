...
import { actions as LocationActions, getLocations, locationType, Location } from "../reducers/locations";
...
const CreateLocationDialog = React.lazy(
    () => import(/* webpackChunkName: "CreateLocationDialog" */ "./dialogs/CreateLocationDialog")
);
...
const EditLocationDialog = React.lazy(
    () => import(/* webpackChunkName: "CreateLocationDialog" */ "./dialogs/EditLocationDialog")
);
...
interface Props {
    ...
    createDirectoryIndex: (path: string, fullTextIndex: boolean, isCurrentLocation: boolean) => void;
...
class LocationManager extends React.Component<Props, State> {
    ...
    indexLocation = () => {
        ...
        const isCurrentLocation =
            selectedLocation && selectedLocation.uuid && selectedLocation.uuid === currentLocationId;

        if (selectedLocation.type === locationType.TYPE_CLOUD) {
            PlatformIO.enableObjectStoreSupport(selectedLocation)
                .then(() => {
                    createDirectoryIndex(selectedLocation.paths[0], selectedLocation.fullTextIndex, isCurrentLocation);
                    return true;
                })
                .catch(() => {
    ...
    handleFileMoveDrop = (item, monitor) => {
        if (monitor) {
            ...
            if (this.props.isReadOnlyMode) {
                this.props.showNotification(i18n.t("core:dndDisabledReadOnlyMode"), "error", true);
                return;
            }
            if (!AppConfig.isWin && !path.startsWith("/")) {
                this.props.showNotification(i18n.t("Moving file not possible"), "error", true);
                return;
            }
        ...
        return (
            <div key={location.uuid}>
                <ListItem
                    ...
                    title={
                        location.isDefault
                            ? i18n.t("core: thisIsStartupLocation") + " : " + location.paths[0]
                            : location.paths[0]
                    }
                    button
                    ...
                    {!AppConfig.locationsReadOnly && (
                        <ListItemSecondaryAction>
                            <IconButton
                                ...
                                onContextMenu={(event) => this.handleLocationContextMenuClick(event, location)}
                            >
                                {location.isDefault && <DefaultLocationIcon data-tid="startupIndication" />}
                                <MoreVertIcon />
                            </IconButton>
                        </ListItemSecondaryAction>
                    )}
        ...
        return (
            <div className={classes.panel} style={this.props.style}>
                ...
                <div className={classes.toolbar}>
                    <Typography className={classNames(classes.panelTitle, classes.header)} variant="subtitle1">
                        {i18n.t("core:locationManager")}
                    </Typography>
                    <IconButton data-tid="locationManagerMenu" onClick={this.handleLocationManagerMenu}>
                        <MoreVertIcon />
                    </IconButton>
                </div>
                <LocationManagerMenu
                ...
                <div>
                    ...
                    {this.state.isDeleteLocationDialogOpened && (
                        <ConfirmDialog
                            ...
                            content={i18n.t("core:deleteLocationContentAlert", {
                                locationName: this.state.selectedLocation ? this.state.selectedLocation.name : "",
                            })}
                            confirmCallback={(result) => {
                                if (result && this.state.selectedLocation) {
                                    this.props.removeLocation(this.state.selectedLocation);
                                    this.directoryTreeRef[this.state.selectedLocation.uuid].removeLocation();
                                }
                            }}
                    ...
                    >
                        <MenuItem data-tid="editLocation" onClick={this.showEditLocationDialog}>
                            <ListItemIcon>
                                <EditIcon />
                            </ListItemIcon>
                        ...
                        <MenuItem data-tid="moveLocationDown" onClick={this.moveLocationDown}>
                            <ListItemIcon>
                                <ArrowDownwardIcon />
                            </ListItemIcon>
                            <ListItemText primary={i18n.t("core:moveDown")} />
                        </MenuItem>
                        <MenuItem data-tid="removeLocation" onClick={this.showDeleteLocationDialog}>
                            <ListItemIcon>
                                <DeleteIcon />
                            </ListItemIcon>
                            <ListItemText primary={i18n.t("core:removeLocation")} />
                        </MenuItem>
                        <MenuItem data-tid="showInFileManager" onClick={this.showInFileManager}>
                            <ListItemIcon>
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(LocationManager));
