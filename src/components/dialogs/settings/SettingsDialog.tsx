...
const SettingsDialog = (props: Props) => {
    ...
    const [isResetSettingsDialogOpened, setIsResetSettingsDialogOpened] = useState<boolean>(false);
    const settingsFileTypeRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        const initSupportedFileTypes = props.supportedFileTypes.reduce((accumulator, fileType) => {
    ...
    const onAddFileType = (item = defaultFileTypeObject) => {
        setItems([...items, item], () => {
            if (settingsFileTypeRef && settingsFileTypeRef.current) {
                const lastFileType = settingsFileTypeRef.current.querySelector("li:last-child");
                lastFileType.scrollIntoView({ behavior: "smooth" });
            }
        });
    };

    const updateItems = (identifierKey, identifierValue, targetKey, targetValue, disableSave = false) => {
    ...
    const renderTitle = () => (
        <React.Fragment>
            ...
            <AppBar position="static" color="default">
                <Tabs value={currentTab} onChange={handleTabClick} indicatorColor="primary" variant="fullWidth">
    ...
    const renderContent = () => (
        <DialogContent className={props.classes.mainContent}>
            ...
            <div data-tid="settingsDialog" className={props.classes.mainContent} ref={settingsFileTypeRef}>
                {currentTab === 0 && <SettingsGeneral showResetSettings={setIsResetSettingsDialogOpened} />}
                {currentTab === 1 && (
                    <SettingsFileTypes
                        items={items}
    ...
    const renderActions = () => (
        ...
        >
            ...
            <Button data-tid="closeSettingsDialog" onClick={props.onClose} color="primary">
...
const mapStateToProps = (state) => ({
    supportedFileTypes: state.settings.supportedFileTypes,
});
