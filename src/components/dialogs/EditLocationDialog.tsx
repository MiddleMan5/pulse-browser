...
class EditLocationDialog extends React.Component<Props, State> {
    constructor(props) {
        ...
        if (props.open === true && props.location) {
            ...
            if (props.location.type === locationType.TYPE_CLOUD) {
                properties = {
                    storeName: dir ? extractDirectoryName(dir, PlatformIO.getDirSeparator()) : props.location.name,
                    storePath: dir || props.location.paths[0],
                };
            } else {
                properties = {
                    // TODO maybe its better to separate name/path keys for different locationTypes ??
                    name: dir ? extractDirectoryName(dir, PlatformIO.getDirSeparator()) : props.location.name,
    ...
    renderTitle = () => <DialogTitle>{i18n.t("core:editLocationTitle")}</DialogTitle>;
