...
class ImportExportTagGroupsDialog extends React.Component<Props, State> {
    ...
    renderTagGroups = (tagGroup, index) => (
        <div key={tagGroup.uuid || tagGroup.key}>
            ...
            <TagGroupContainer taggroup={tagGroup}>
                {tagGroup.children && tagGroup.children.map((tag: Tag) => <TagContainer tag={tag} tagMode="display" />)}
            </TagGroupContainer>
        </div>
    );
    ...
    renderContent = () => (
        <DialogContent className={this.props.classes.root}>
            ...
            <FormControl fullWidth={true}>{this.state.tagGroupList.map(this.renderTagGroups)}</FormControl>
        </DialogContent>
    );
