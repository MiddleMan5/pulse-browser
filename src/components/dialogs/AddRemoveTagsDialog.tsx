...
class AddRemoveTagsDialog extends React.Component<Props, State> {
    ...
    handleChange = (name: string, value: Array<Tag>, action: string) => {
        if (action === "remove-value") {
            ...
            const newlyAddedTags = this.state.newlyAddedTags.filter((tag) => !tagsToRemove.includes(tag.title));
            this.setState({
                newlyAddedTags,
            });
        } else {
    ...
    render() {
        const { open, selectedEntries = [], removeTags, removeAllTags, fullScreen, onClose } = this.props;
        const { newlyAddedTags = [] } = this.state;
        const disabledButtons = !newlyAddedTags || newlyAddedTags.length < 1 || selectedEntries.length < 1;

        return (
            <Dialog open={open} fullScreen={fullScreen} onClose={onClose} keepMounted scroll="paper">
                            ...
                            selectedEntries.map((entry) => (
                                <ListItem key={uuidv1()} title={entry.path}>
                                    ...
                                    <Typography variant="inherit" noWrap>
                                        {extractFileName(entry.path || "", PlatformIO.getDirSeparator())}
                                    </Typography>
                                </ListItem>
                            ))}
                    </List>
                ...
                <DialogActions>
                    <Button data-tid="cancelTagsMultipleEntries" onClick={() => this.onClose()} color="primary">
                        {i18n.t("core:cancel")}
                    </Button>
                    <Button
