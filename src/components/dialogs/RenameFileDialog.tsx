...
const RenameFileDialog = (props: Props) => {
    ...
    let fileName = extractFileName(props.selectedFilePath, PlatformIO.getDirSeparator());
    ...
    const onConfirm = () => {
        if (!inputError) {
            const fileDirectory = extractContainingDirectoryPath(props.selectedFilePath, PlatformIO.getDirSeparator());
            const newFilePath = fileDirectory + PlatformIO.getDirSeparator() + fileName;
    ...
    const renderTitle = () => <DialogTitle>{i18n.t("core:renameFileTitle")}</DialogTitle>;

    const renderContent = () => (
        <DialogContent className={props.classes.root}>
            <FormControl data-tid="renameFileDialog" fullWidth={true} error={inputError}>
                <TextField
                    error={inputError}
                    margin="dense"
                    name="fileName"
    ...
    const renderActions = () => (
        <DialogActions>
            <Button data-tid="closeRenameFileDialog" onClick={() => props.onClose()} color="primary">
                {i18n.t("core:cancel")}
            </Button>
            <Button disabled={inputError} onClick={onConfirm} data-tid="confirmRenameFileDialog" color="primary">
                {i18n.t("core:ok")}
            </Button>
        </DialogActions>
    );
