...
import { AttributionControl, Map, LayerGroup, Marker, Popup, TileLayer, withLeaflet } from "react-leaflet";
...
const EntryProperties = (props: Props) => {
    ...
    const parentDirectoryPath = extractContainingDirectoryPath(props.openedEntry.path, PlatformIO.getDirSeparator());
    ...
    customRenderer.image = (href, title, text) => {
        ...
        if (!sourceUrl.startsWith("http")) {
            sourceUrl = parentDirectoryPath + PlatformIO.getDirSeparator() + sourceUrl;
        }
    ...
    const entryName = props.openedEntry.isFile
        ...
        : extractDirectoryName(props.openedEntry.path, PlatformIO.getDirSeparator());

    const currentEntry = enhanceOpenedEntry(props.openedEntry, props.tagDelimiter);
    ...
    const [isMoveCopyFilesDialogOpened, setMoveCopyFilesDialogOpened] = useState<boolean>(false);
    const [isFileThumbChooseDialogOpened, setFileThumbChooseDialogOpened] = useState<boolean>(false);
    ...
    const renameEntry = () => {
        if (editName !== undefined) {
            ...
            const path = extractContainingDirectoryPath(currentEntry.path, PlatformIO.getDirSeparator());
    ...
    const toggleThumbFilesDialog = () => {
        ...
        if (!currentEntry.editMode && editName === undefined && editDescription === undefined) {
    ...
    if (currentEntry.isFile) {
        thumbPath = getThumbFileLocationForFile(currentEntry.path, PlatformIO.getDirSeparator());
    } else {
        thumbPath = getThumbFileLocationForDirectory(currentEntry.path, PlatformIO.getDirSeparator());
    }
    const thumbPathUrl = thumbPath ? 'url("' + thumbPath + "?" + new Date().getTime() + '")' : "";
    // if (AppConfig.isWin) {
    //   thumbPathUrl = thumbPathUrl.split('\\').join('\\\\');
    // }

    const ldtm = currentEntry.lmdt
        ? new Date(currentEntry.lmdt).toISOString().substring(0, 19).split("T").join(" ")
        : "";
    ...
    function getMenuItem(perspective) {
        ...
        return (
            <MenuItem key={perspective} value={perspective}>
                {icon}
                <ListItemText primary={perspective.charAt(0).toUpperCase() + perspective.slice(1)} />
            </MenuItem>
        );
    }
            ...
            <Grid container>
                <Grid item xs={12}>
                    <div className={classes.fluidGrid}>
                        ...
                        {!isReadOnlyMode && !currentEntry.editMode && editDescription === undefined && (
                ...
                {geoLocation && (
                    <Grid item xs={12}>
                        ...
                        >
                            <TileLayer attribution={MB_ATTR} url="https://{s}.tile.osm.org/{z}/{x}/{y}.png" />
                            <LayerGroup>
                                <Marker icon={iconFileMarker} position={[geoLocation.lat, geoLocation.lng]}>
                                    <Popup
                                        style={{
                    ...
                    <div className={classes.fluidGrid}>
                        ...
                        {!isReadOnlyMode && !currentEntry.editMode && editName === undefined && (
                            <div className={classes.gridItem} style={{ textAlign: "right" }}>
                                ...
                                <Button color="primary" className={classes.button} onClick={toggleEditDescriptionField}>
                                    {editDescription !== undefined
                                        ? i18n.t("core:confirmSaveButton")
                                        : i18n.t("core:edit")}
                                </Button>
                ...
                <Grid item xs={12}>
                    <div className={[classes.fluidGrid, classes.ellipsisText].join(" ")}>
                        <div className={classes.gridItem} style={{ width: "50%", alignSelf: "baseline" }}>
                            <Typography variant="caption" className={classes.header} style={{ display: "block" }}>
                        ...
                        {currentEntry.isFile ? (
                            ...
                            <div className={classes.gridItem} style={{ width: "50%" }}>
                                <Typography variant="caption" style={{ display: "block" }} className={classes.header}>
                                ...
                                <FormControl fullWidth={true} className={classes.formControl}>
                                    <TransparentBackground>
                                        <Button
                                            fullWidth={true}
                                            className={[classes.colorChooserButton, classes.button].join(" ")}
                                            style={{
                ...
                <Grid item xs={12}>
                    <div className={classes.fluidGrid}>
                        ...
                        {currentEntry.isFile &&
                            ...
                            editName === undefined &&
                            editDescription === undefined && (
                                <Button color="primary" className={classes.button} onClick={toggleMoveCopyFilesDialog}>
                                    {i18n.t("core:move")}
                                </Button>
                            )}
                    </div>
                            ...
                            InputProps={{
                                ...
                                startAdornment: (
                                    <InputAdornment position="start">
                                        {currentEntry.url ? <CloudLocationIcon /> : <LocationIcon />}
                                    </InputAdornment>
                                ),
                            }}
                        />
                    ...
                    <Grid item xs={12}>
                        ...
                        <FormControl fullWidth={true} className={classes.formControl}>
                            ...
                            >
                                ...
                                {Object.values(perspectives).map((perspective) => getMenuItem(perspective))}
                                {/* {Pro &&
                  Object.values(
                ...
                <Grid item xs={12}>
                    <div className={classes.fluidGrid}>
                        ...
                        {!isReadOnlyMode &&
                            ...
                            editName === undefined &&
                            editDescription === undefined && (
                                <Button color="primary" className={classes.button} onClick={toggleThumbFilesDialog}>
...
export default withLeaflet(withStyles(styles, { withTheme: true })(EntryProperties));
