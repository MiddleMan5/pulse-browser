...
const Search = React.memo((props: Props) => {
    ...
    const [searchBoxing, setSearchBoxing] = useState<"location" | "folder" | "global">("location");
    ...
    const handleLastModifiedChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const { target } = event;
        const { value, name } = target;
    ...
    function removeTags(tagsArray, removeTagsArray) {
        ...
        return tagsArray.filter((tag) => removeTagsArray.some((valueTag) => valueTag.title !== tag.title));
    }
    ...
    function haveSearchFilters(searchQuery) {
        return (
            ...
            (searchQuery.fileTypes !== undefined && searchQuery.fileTypes !== FileTypeGroups.any) ||
    ...
    const handleTimePeriodChange = (event) => {
        ...
        if (toDateTime && fromDateTime) {
            tagTPeriodHelper = "From: " + formatDateTime(fromDateTime) + " To: " + formatDateTime(toDateTime);
        } else {
            tagTPeriodHelper = "";
        }
    ...
    const switchSearchBoxing = (event: React.MouseEvent<HTMLElement>, boxing: "location" | "folder" | "global") => {
        if (boxing !== null) {
    ...
    const indexStatus = indexedEntriesCount ? indexedEntriesCount + " indexed entries" : "";
    return (
        <div className={classes.panel} style={props.style}>
            ...
            <div className={classes.toolbar}>
                ...
                <IconButton style={{ marginLeft: "auto" }} data-tid="searchMenu" onClick={handleSearchMenu}>
                    <MoreVertIcon />
                </IconButton>
            </div>
            <SearchMenu
            ...
            <div className={classes.searchArea}>
                <FormControl className={classes.formControl} style={{ marginTop: 10 }} disabled={indexing}>
                    <OutlinedInput
                        id="textQuery"
                        name="textQuery"
                        value={textQuery}
                ...
                <FormControl className={classes.formControl} disabled={indexing}>
                    ...
                    >
                        ...
                        <ToggleButton value="folder" title={i18n.t("searchCurrentFolderWithSubFolders")}>
                            {i18n.t("folder")}
                        </ToggleButton>
                        <ToggleButton
                ...
                <FormControlLabel
                    ...
                    label={
                        <Typography style={{ padding: 15, color: props.theme.palette.text.primary }}>
                ...
                {AppConfig.showAdvancedSearch && (
                    <React.Fragment>
                        ...
                        >
                            ...
                            >
                                <MenuItem value={FileTypeGroups.any}>{i18n.t("core:anyType")}</MenuItem>
                                ...
                                <MenuItem value={FileTypeGroups.images} title={FileTypeGroups.images.toString()}>
                                    <IconButton>
                                        <PictureIcon />
                                    </IconButton>
                                    {i18n.t("core:searchPictures")}
                                </MenuItem>
                                <MenuItem value={FileTypeGroups.documents} title={FileTypeGroups.documents.toString()}>
                                    <IconButton>
                                        <DocumentIcon />
                                    </IconButton>
                                    {i18n.t("core:searchDocuments")}
                                </MenuItem>
                                <MenuItem value={FileTypeGroups.notes} title={FileTypeGroups.notes.toString()}>
                                    <IconButton>
                                        <NoteIcon />
                                    </IconButton>
                                    {i18n.t("core:searchNotes")}
                                </MenuItem>
                                <MenuItem value={FileTypeGroups.audio} title={FileTypeGroups.audio.toString()}>
                                    <IconButton>
                                        <AudioIcon />
                                    </IconButton>
                                    {i18n.t("core:searchAudio")}
                                </MenuItem>
                                <MenuItem value={FileTypeGroups.video} title={FileTypeGroups.video.toString()}>
                                    <IconButton>
                                        <VideoIcon />
                                    </IconButton>
                                    {i18n.t("core:searchVideoFiles")}
                                </MenuItem>
                                <MenuItem value={FileTypeGroups.archives} title={FileTypeGroups.archives.toString()}>
                                    <IconButton>
                                        <ArchiveIcon />
                                    </IconButton>
                                    {i18n.t("core:searchArchives")}
                                </MenuItem>
                                <MenuItem value={FileTypeGroups.bookmarks} title={FileTypeGroups.bookmarks.toString()}>
                                    <IconButton>
                                        <BookmarkIcon />
                                    </IconButton>
                                    {i18n.t("core:searchBookmarks")}
                                </MenuItem>
                                <MenuItem value={FileTypeGroups.ebooks} title={FileTypeGroups.ebooks.toString()}>
                                    <IconButton>
                                        <BookIcon />
                                    </IconButton>
                        ...
                        >
                            ...
                            >
                                ...
                                <MenuItem value="past6Months">{i18n.t("core:past6Months")}</MenuItem>
                                <MenuItem value="pastYear">{i18n.t("core:pastYear")}</MenuItem>
                                <MenuItem value="moreThanYear">{i18n.t("core:moreThanYear")}</MenuItem>
                            </Select>
                        </FormControl>
                        <FormControl
                            className={classes.formControl}
