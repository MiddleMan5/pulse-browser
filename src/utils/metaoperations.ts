import { FileSystemEntryMeta, loadMetaDataPromise, saveMetaDataPromise } from "../services/utils-io";

// eslint-disable-next-line import/prefer-default-export
export function savePerspective(path: string, perspective: string): Promise<FileSystemEntryMeta> {
        ...
        loadMetaDataPromise(path)
            .then((fsEntryMeta: FileSystemEntryMeta) => {
                ...
                saveMetaDataPromise(path, updatedFsEntryMeta)
                    ...
                    .catch((err) => {
                        console.warn("Error adding perspective for " + path + " with " + err);
                        reject();
                    });
                return true;
            })
            .catch(() => {
                ...
                saveMetaDataPromise(path, newFsEntryMeta)
                    ...
                    .catch((error) => {
                        console.warn("Error adding perspective for " + path + " with " + error);
                        reject();
                    });
            });
    });
