...
export function parseLatLon(latLongInput: string): { lat: number; lon: number } | false {
    ...
    if (
        !/^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?),\s*[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)$/.test(cleanedInput)
    ) {
        return false;
    }
...
export function immutablySwapItems(items: any, firstIndex: number, secondIndex: number) {
 ...
 * @returns {Blob}
 */
export function b64toBlob(b64Data: any, contentType: string = "", sliceSize: number = 512) {
...
export function formatFileSize2(sizeInBytes: number, siSystem: boolean): string {
    ...
    const units = siSystem ? ["kB", "MB", "GB", "TB", "PB", "EB"] : ["KiB", "MiB", "GiB", "TiB", "PiB", "EiB"];
    let cUnit = -1;
    do {
...
export function formatDateTime4Tag(date: string | Date, includeTime: boolean, includeMS?: boolean): string {
    if (date === undefined || date === "") {
        return "";
    }
    const d = new Date(date);
 ...
 * @returns {boolean}
 */
export const isObj = (val: any) => typeof val === "object" && !isArr(val) && !isNull(val);
 ...
 */
export const hasProp = (obj: Object, prop: any): boolean => obj.hasOwnProperty(prop);

/**
 * Check if object has method
 */
export const hasMethod = (obj: Object, method: any): boolean => hasProp(obj, method) && isFunc(obj[method]);

/**
 * Check if object has key
 */
export const hasKey = (obj: Object, key: any): boolean => getKeys(obj).indexOf(key) > -1;
 ...
 * @param direction
 */
export const sortBy = (items: any, keysText: any, type: string = "string", direction: string = "asc") =>
    items.sort((a, b) => {
