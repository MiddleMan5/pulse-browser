...
export function isYearPeriod(tagDate: string): boolean {
    return /(^|\s)([0123][0123456789][0123456789][0123456789]-[0123][0123456789][0123456789][0123456789])(\s|$)/.test(
        tagDate
    );
}
...
export function extractTimePeriod(value: string): { fromDateTime: Date | null; toDateTime: Date | null } {
    ...
    if (value.length && !/(^|\s)([0123456789])(\s|$)/.test(value.substr(0, 1))) {
        return {
            // Return if the first char is not a number, ignoring values starting with letters
            fromDateTime,
            toDateTime,
        };
    }
    try {
        if (isYear(value)) {
            ...
            toDateTime = new Date(
                year + "-" + month + "-" + getDaysInMonth(parseInt(year, 10), parseInt(month, 10)) + " 23:59:59.999"
            );
            ...
            fromDateTime = new Date(
                fromYear + "-" + fromMonth + "-" + fromDay + " " + fromHour + ":" + fromMin + ":00"
            );
            ...
            fromDateTime = new Date(
                fromYear + "-" + fromMonth + "-" + fromDay + " " + fromHour + ":" + fromMin + ":" + fromSec
            );
            toDateTime = new Date(
                toYear + "-" + toMonth + "-" + toDay + " " + toHour + ":" + toMin + ":" + toSec + ".999"
            );
        }
    } catch (err) {
...
export function parseFullDate(date: string) {
    ...
    const parsedDate = date.match(/^(\d{4})(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$/);
    if (parsedDate) {
        // make a date
