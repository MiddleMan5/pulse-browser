...
export function baseName(
    ...
    const fileName = dirPath.substring(dirPath.lastIndexOf(dirSeparator) + 1, dirPath.length);
...
export function getMetaDirectoryPath(
    ...
    return (directoryPath ? normalizePath(directoryPath) + dirSeparator : "") + AppConfig.metaFolder;
}
...
export function getThumbFileLocationForDirectory(
    ...
    return entryPath + dirSeparator + AppConfig.metaFolder + dirSeparator + AppConfig.folderThumbFile;
}
...
export function extractFileName(
    ...
    return filePath ? filePath.substring(filePath.lastIndexOf(dirSeparator) + 1, filePath.length) : filePath;
}
...
export function extractDirectoryName(
    ...
    if (dirPath.indexOf(dirSeparator) !== -1) {
        if (dirPath.endsWith(dirSeparator)) {
            directoryName = directoryName.substring(0, dirPath.lastIndexOf(dirSeparator));
        }
        const lastDirSeparator = directoryName.lastIndexOf(dirSeparator);
        if (lastDirSeparator !== -1) {
            directoryName = directoryName.substring(lastDirSeparator + 1, directoryName.length);
        }
    }
...
export function extractContainingDirectoryName(filePath: string, dirSeparator: string) {
